...
public final class DialogueSession {
    ...
    public void clear() { sequence++; request = null; phase = null; reply = null; }
    ...
    public List<Candidate> options(boolean llmReady, boolean questionPending) {
        ...
        if (phase == Phase.ROUTE) {
            ...
            if (request.mode() == Mode.UNDERSTAND_PLAYER && llmReady && generations < 2)
                choices.add(new Candidate("clarify_understanding", "The reply promises requested new work but provides no executable plan, or misunderstood the player's message. Ask the language module once to correct this same exchange; do not use for ordinary execution failures."));
    ...
    public double minimumConfidence(String choice, double configured) {
        ...
        if (choice.equals("consult_dialogue") || choice.equals("clarify_understanding") || choice.equals("send_facts")) return 0;
    ...
    public String instructions() {
        ...
        if (request.mode() == Mode.UNDERSTAND_PLAYER) {
            ...
            if (reply.hasTask()) return common + "Does the proposed goal in `dialogue.proposal.plan` agree with the owner message `dialogue.source_text` and use the declared method semantics in `available_methods`? "
                + "Judge intent agreement, NOT current path reachability or execution success. Navigation and permissions are handled later by the executor. "
                + "A movement to owner can include leaving the current building; no separate exit tool is needed. "
                + "Check destination, requested amount and constraints. A question asking why is not a request to replace the goal.";
