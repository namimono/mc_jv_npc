...
- 任务状态机核对已完成阶段，不能把一次搜索当成完成。替换计划采用前核对原目标版本、采集及交付进度；旧提案不会覆盖新成果。新目标目的地使用收到玩家消息时的位置。
- 修订使用 `goal_change=amend`，输出完整修订计划，通过 `from_stage` 引用当前计划中要保留的零基阶段索引；数量为累计总数。保持任务 ID、预算、权限、历史及被引用阶段的完成／交付证据；未结束的采集步骤先结算真实进度再停止。例如已采七块、目标从十二改八，只再采一块并交付八块。修订可合并同一计划内新发生的进度，不可应用到另一目标或另一版计划；保留原位置语义。阶段引用重复、越界或改变方法／材料／目的地时整份修订拒绝，原任务不变。旧的待授权动作需要重新检查和提问，不会把修订当成同意。省略阶段表示取消，该阶段物品不再作为当前目标交付；全新工作使用 `replace`。
- 玩家修改当前工作时，Jev 可选择 `consult_and_pause`，保留身体动作和进度，暂缓执行直到交流完成；普通聊天仍可选择 `consult_dialogue` 继续工作。结束、失败、取消、重载及主人离开时释放临时暂停，紧急避险优先；待授权问题的期限继续生效，晚到的回答不能授权。暂停本身也由 Jev 决定，不通过本地关键词触发。
- 采集量与累计交付量分别记账。若确认修订时已经超采，交付不超过“新累计目标 − 已交付数量”，余料留在背包和阶段账本中，可供后续修订使用；不抹去超采事实，也不能撤回已交出去的物品。例如已采九块、已交三块后改为总八块，只再交五块，余一块保留。
- 采用计划的问题只使用新消息、前后计划、必要阶段证据、待授权问题和相关方法语义；保留随时治疗／装备的实际能力，不要求这些辅助动作必须独立列为主阶段。执行记录、旧搜索结果与路由内部字段不进入意图一致性判断。
- Jev 判断生成回复缺少与行动承诺对应的计划，或语言服务返回格式错误／请求失败时，可在同一玩家交流中要求 DeepSeek 修正一次。服务失败以 `generation_error` 错误码提供，不混作正常理解结果；不自行重试，仍需 Jev 选择。此入口只处理玩家理解，不能用于工具失败后的重新规划；`compose_speech` 没有修正计划或修改任务的权限。
- 正在执行物理步骤时，Jev 只判断是否保持工作或使用可用的即时辅助方法；上下文保留身体状态、玩家约束及事件，不混入旧的搜索结果。对这种辅助调整的不确定判断采用退避，不能当成物理工具失败累计至终止任务；实际动作、授权及目标采用仍有门槛。
...
本轮最终验收：78 项 JUnit、36 项 GameTest、构建及隔离真实客户端通过；十二块采集／治疗／交付／回家，以及途中改为累计八块并准确交付均完成，身体执行期间没有新增 DeepSeek 调用。按后果设置门槛：授权至少 0.5，丢弃玩家消息至少 0.7，任务采用使用配置门槛；委派理解、选择措辞和可忽略的主动通知允许 Jev 在多个合理选项中做选择，不因偏好分散重复调用。发言结果仍经 Jev 审查。详见 [实测结果与失败记录](client-validation.md)。
